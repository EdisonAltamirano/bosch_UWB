...
def render_aoa_frame(
    ...
    ax_polar.set_title(f"AoA tracks  frame={frame_idx}  t={timestamp_s:.2f}s")
    ...
    for track_aoa in track_aoas:
        ...
        ax_polar.scatter(np.deg2rad(track_aoa.angle_deg), track_aoa.range_m, s=40, color=[color])
        ax_polar.text(
            np.deg2rad(track_aoa.angle_deg),
            track_aoa.range_m,
            f"#{track_aoa.track_id}",
            fontsize=8,
            color=color,
        )
    ...
    ax_heatmap.set_title("Range-angle heatmap (dB)")
...
def save_aoa_animation(
    ...
    print(f"[aoa] writing {total_frames} frames to {partial_output_path.name}")
    try:
        with imageio.get_writer(str(partial_output_path), fps=fps, codec="libx264") as writer:
            for frame_idx, timestamp_s in enumerate(timestamps_s):
                if frame_idx == 0 or frame_idx == total_frames - 1 or frame_idx % 25 == 0:
                    print(f"[aoa] rendering frame {frame_idx + 1}/{total_frames}")
